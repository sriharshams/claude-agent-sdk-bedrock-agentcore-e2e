"""Chat manager and streaming for the Streamlit frontend."""
...
class ChatManager:
    ...
    def invoke_endpoint_nostreaming(
        ...
        """Invoke agent endpoint using HTTP request with bearer token (no streaming)."""
        escaped_arn = urllib.parse.quote(agent_arn, safe="")
        url = f"https://bedrock-agentcore.{st.session_state['region']}.amazonaws.com/runtimes/{escaped_arn}/invocations"
    ...
    def invoke_endpoint(
        ...
        """Invoke agent endpoint using HTTP request with bearer token (streaming)."""
        escaped_arn = urllib.parse.quote(agent_arn, safe="")
        url = f"https://bedrock-agentcore.{st.session_state['region']}.amazonaws.com/runtimes/{escaped_arn}/invocations"
    ...
    def display_chat_history(self):
        ...
        for message in messages_to_show:
            ...
            emoji = "U" if message["role"] == "user" else "A"
    ...
    def process_user_message(self, prompt: str, actor_id: str, bearer_token: str):
        ...
        with st.chat_message("assistant"):
            ...
            create_safe_markdown_text(
                f'<div class="assistant-bubble">{clickable_streaming_text}<br><span style="font-size:0.9em;color:#888;">Response time: {elapsed:.2f} seconds</span></div>',
                message_placeholder,
            )
